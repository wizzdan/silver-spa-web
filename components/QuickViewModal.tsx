    ...
    <div className="fixed inset-0 bg-brand-black bg-opacity-70 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-brand-cream rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto flex flex-col md:flex-row animate-fade-in" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="w-full md:w-1/2 p-8 flex flex-col justify-center relative">
          <button onClick={onClose} className="absolute top-4 right-4 p-1 rounded-full text-brand-black hover:text-brand-gold active:bg-brand-black/10 transition-all">
            <CloseIcon />
          </button>
          ...
          <button className="bg-brand-gold text-brand-black font-semibold py-3 px-8 rounded-full uppercase tracking-wider transition-all duration-300 hover:bg-brand-black hover:text-brand-cream hover:shadow-lg transform hover:-translate-y-1 active:scale-95 w-full text-sm md:text-base">
            Book Now
          </button>
        </div>
      </div>
